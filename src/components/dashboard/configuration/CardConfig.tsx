...
export default function CardConfig() {
  ...
  const openModal = (content: string) => {
    ...
    const savedFormData = JSON.parse(
      localStorage.getItem(`submittedData_${content}`) || "{}"
    );
    setFormData(savedFormData);

    const existingSubmittedCategories = JSON.parse(
      localStorage.getItem("submittedCategories") || "[]"
    );
  ...
  const nextPage = () => {
    ...
    setProgress(
      ((currentPage + 1) /
        Math.ceil(cardData[modalContent].length / questionsPerPage)) *
        100
    );
  };
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const requiredFields = cardData[modalContent].filter(
            ...
            <form onSubmit={handleSubmit}>
              <Progress
                ...
                progress={Math.round(
                  (currentPage /
                    Math.ceil(
                      cardData[modalContent].length / questionsPerPage
                    )) *
                    100
                )}
              />
                        ...
                        {field.options?.map((option, optionIndex) => (
                          ...
                          >
                            <input
                              ...
                              value={option}
                              checked={
                                formData[`field_${field.label}`] === option
                              }
                              onChange={handleInputChange}
                        ...
                        {field.options.map((option, optionIndex) => (
                          ...
                          >
                            <input
                              ...
                              checked={
                                formData[
                                  `field_${field.label}_${optionIndex}`
                                ] === "true"
                              }
                              onChange={handleInputChange}
                              className="text-blue-500"
